...
class Test_Sessions(pytest_common.TestBase):
    ...
    @pytest.mark.parametrize('test_instance', [
      {
        ...
        "message" : "0  Check Inactive Sessions       : \x1b[32m\x1b[01mPASS\x1b[0m  1/1 port 5060 sessions have idle times < 400s (2 flows processed)"
      },
      {
        "json_reply" : {
           'data': {
        ...
        "message" :"0  Check Inactive Sessions       : \x1b[31m\x1b[01mFAIL  2/2 port 5060 sessions idle > 400s (3 flows processed)\x1b[0m"
      }
    ]
   )
    ...
    def test_run_on_cond(self, test_instance, local_context, graphql_url, capsys):
        ...
        router_context.update(pytest_common.assets[1])
